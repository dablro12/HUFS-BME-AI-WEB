...
class App extends Component {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      searchKeyword: '',
      mode: "",
      inpaintingType: "ocigan", // Inpainting 타입 추가
      anchorEl: null, // Dropdown 메뉴를 열고 닫기 위한 anchorEl 추가
      nickname: ""
    } 
  }
  ...
  componentDidMount() { //서버 api에서 data를 받아오는 함수
    fetch("/authcheck")
      ...
      .then((json) => {        
        if (json.isLogin === "True") {
          this.setState({ mode: "WELCOME", nickname: json.nickname });
        } else {
          this.setState({ mode: "LOGIN", nickname: "" });
        }
      });
    this.timer = setInterval(this.progress, 800); //1000이 1초 -> 20은 0.02초
    this.callApi()
  ...
  render() {
    ...
    const filteredComponents = (data) => {
      ...
      return data.map((c,index) => {
        return <Customer stateRefresh={this.stateRefresh} key = {c.id} id = {c.id} num = {index + 1 } image={c.image} inpainted={c.image2} mask={c.mask} name={c.name} explanation={c.explanation} uploader={c.user} />
      });
    }
    const cellList = ["Number", "Original Image", "Mask Image", "Inpainted Image","Name", "Explanation","Uploader", "Setting"];
    ...
    if(this.state.mode === "LOGIN"){
      ...
      content = <>
        <div style={{ marginLeft: 10, marginRight: 10 }}>
          ...
          <DataAdd style={{ marginTop: 20, marginBottom: 20, display: 'flex', alignItems: "center", justifyContent: "center" }} stateRefresh={this.stateRefresh} nickname={this.state.nickname} />  
          ...
          <Menu
            ...
            onClose={this.handleDropdownClose}
          >
            <MenuItem onClick={() => this.handleDropdownSelect("ocigan")}>Ocigan</MenuItem>
            <MenuItem onClick={() => this.handleDropdownSelect("vae")}>Vae</MenuItem>
            {/* <MenuItem onClick={() => this.handleDropdownSelect("type3")}>Type 3</MenuItem> */}
          </Menu> &nbsp;
          <Button
            variant="contained"
